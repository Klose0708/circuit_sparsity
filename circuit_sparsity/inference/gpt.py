...
def load_model(model_path, flash=False, grad_checkpointing=False, cuda=True):
    ...
    # --- FIX: Filter out unknown keys for compatibility ---
    valid_keys = set(GPTConfig.__dataclass_fields__.keys())
    filtered_config = {k: v for k, v in beeg_config_json.items() if k in valid_keys}
