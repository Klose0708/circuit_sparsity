...
def main():
    ...
    with datasetcol:
        ...
        def get_dataset_options_for_base_path(base_path):
            try:
                # FIX: Force forward slash for URLs on Windows
                url = f"{base_path}/{model_name}"
                opts = list(local_listdir(url))
                if not opts: raise Exception("Empty")
                return opts
            except Exception:
                # Fallback for Windows/Network issues
                if model_name == "csp_yolo1": return ["single_double_quote"]
                if model_name == "csp_yolo2": return ["bracket_counting_beeg"]
                return []
        ...
        # --- FIX START: Safe default selection ---
        default_index = 0
        preferred_datasets = ["single_double_quote", "bracket_counting_beeg", "single_double_quote_beeg3"]
        
        for pref in preferred_datasets:
            if pref in dataset_options:
                default_index = dataset_options.index(pref)
                break
    ...
    with sweepnamecol:
        ...
        def get_sweep_options_for_base_path(base_path):
            try:
                opts = list(local_listdir(f"{base_path}/{model_name}/{dataset_name}"))
                if not opts: raise Exception
                return opts
            except Exception:
                return ["prune_v4"]
    ...
    with kcol:
        ...
        def get_ks_for_base_path(base_path):
            ...
            if "None" in viz_data_path: return []
            try:
                candidate_xs = [
                    x for x in local_listdir(viz_data_path) 
                ...
                return [128, 256, "k_optim"] # Fallback
    ...
    # --- FIX START ---
    if any(x is None for x in [model_name, dataset_name, sweep_name, k]):
        st.warning("Waiting for selection...")
        st.stop()
    # --- FIX END ---
